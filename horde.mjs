...
const setupReadline = (id) => {
  ...
  const onKeyPress = (chunk, key) => {
    if (key && key.name == "c") {
      ...
      cancelTextGeneration(id)
        .then(() => {
          console.log(`Previous job ${id} cancelled!`);
        })
        .catch((error) => {
...
export const generateText = async ({ hordeState, config, genParams }) => {
  ...
  const cleanUp = setupReadline(id);
  ...
  try {
    for (;;) {
      ...
      console.log(
        `wait time = ${data.wait_time}; queue position = ${data.queue_position}; is possible = ${data.is_possible} (press c and then enter to cancel)`
      );
      await wait(3000);
    }
  } finally {
