...
export default ({ user, assistant, messages, config, generationConfig }) => {
  const systemPrompt = `[INST]<<SYS>> Write the next message from "${assistant}" in this roleplaying scenario with "${user}<</SYS>>".`;
  ...
  const assistantName = (attributes = "") =>
    `${assistant}:\n`;
  const exassistantName = (attributes = "") =>
      `${assistant}:\n`;
  const beforeSystem = "\n\n";
  const afterSystem = "\nScenario:";
  const beforeUser = "\n\n";
  ...
  for (const msg of messages) {
    ...
    if (metadata.type === "new-conversation") {
      ...
      prompt.push({
        ...
        content: `${beforeAssistant}${exassistantName()}${content}${afterAssistant}`,
      });
